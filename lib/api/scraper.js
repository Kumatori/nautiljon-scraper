        ...
        if (genre === 'animes') {
            ...
            return {
                ...
                episodes: {
                    ...
                    listEpisodes: $('h2').filter(function () { return $(this).text().indexOf('Épisodes') > -1; }).next().find('.vtop.aleft')[0]?.children.map(e => e)[0].children.map(e => e)[0].children.slice(1).map((e, i) => {
                        return {
                ...
                relations: relations,
                news: {
                    french: $('#fiche_news li').toArray().map(e => {
                        return {
            ...
            return {
                ...
                name: $('.h1titre > [itemprop="name"]')?.text() || null,
                imageUrl: $('.image_fiche.fleft a img').attr('src') ? `https://nautiljon.com${$('.image_fiche.fleft a img').attr('src').replace('/mini', '')}` : null,
                alternateName: $('span').filter(function () { return $(this).text().indexOf('Titre alternatif : ') > -1; }).parent().html()?.split("</span>")[1].trim() || null,
                originalName: $('span').filter(function () { return $(this).text().indexOf('Titre original : ') > -1; }).parent().html()?.split("</span>")[1].trim() || null,
                country: $('.flag').parent().text()?.split(":")[1]?.split("-")[0]?.trim() || null,
                startDate: {
                    VO: $('[itemprop="datePublished"]').text() || null,
                    VF: $('span').filter(function () { return $(this).text().indexOf('Année VF : ') > -1; }).parent().html()?.split("</span>")[1].trim() || null,
                },
                ...
                volumesNumber: {
                    VO: $('span').filter(function () { return $(this).text().indexOf('Nb volumes VO : ') > -1; }).parent().html()?.split("</span>")[1]?.split("(")[0]?.trim() || null,
                    VF: $('span').filter(function () { return $(this).text().indexOf('Nb volumes VF : ') > -1; }).parent().html()?.split("</span>")[1]?.split("(")[0]?.trim() || null,
                },
                status: {
                    VO: $('span').filter(function () { return $(this).text().indexOf('Nb volumes VO : ') > -1; }).parent().html()?.split("</span>")[1]?.split("(")[1]?.slice(0, -1) || null,
                    VF: $('span').filter(function () { return $(this).text().indexOf('Nb volumes VF : ') > -1; }).parent().html()?.split("</span>")[1]?.split("(")[1]?.slice(0, -1) || null,
                },
                age: $('span').filter(function () { return $(this).text().indexOf('Âge conseillé : ') > -1; }).parent().html()?.split("</span>")[1].trim() || null,
                description: $($(".description").html()?.split('<div class="groupe"')[0].trim()).text() || null,
                volumes: $('#edition_0-1 > .unvol a').toArray().map(e => $(e).attr('href'))
            }
        }
